...
static void out_end_arr(Buffer *out, size_t ctx, uint32_t n) {
	assert(*(out->data_begin + ctx - 1) == TAG_ARR);
...
static void entry_del(Entry *ent) {
	// unlink it from the data structures
	entry_set_ttl(ent, -1);
...
// Equality comparision for the top-level hashtable
...
static bool cb_keys(HNode *node, void *arg) {
	Buffer *wbuf = (Buffer *)arg;
	const std::string &key = container_of(node, Entry, node)->key;
	out_str(wbuf, key.data(), key.size());
	return true;
}
...
static bool try_one_request(Conn *conn) {
	...
	response_begin(wbuf, &header_pos);
	do_request(cmd, wbuf);
	response_end(wbuf, header_pos);
...
static void process_timers() {
	...
	while (!heap.empty() && heap[0].val < now_ms) {
		...
		if (nworks++ >= k_max_works) {
			// server should not be stalled if too many keys are expiring at
			// once
			break;
		}
	}
